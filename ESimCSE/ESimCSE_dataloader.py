...
class CollateFunc(object):
    def __init__(self, tokenizer, max_len=256, q_size=160, dup_rate=0.15):
        ...
        self.dup_rate = dup_rate  # duplicate rate
    ...
    def word_repetition_normal(self, batch_text):
        ...
        for text in batch_text:
            ...
            for index, word in enumerate(text):
                dup_text += word
                if index in dup_word_index:  # sample list 中今进行复制
                    dup_text += word
    ...
    def negative_samples(self, batch_src_text):
        ...
        if len(self.q) > 0:  # 队列已经存在数值
            negative_samples = self.q[:self.q_size]  # 取头部 q_size 个
            # print("size of negative_samples", len(negative_samples))
        
        if len(self.q) + batch_size >= self.q_size:  # 更新队列
            del self.q[:batch_size]  # 删除头部 q_size 个
    ...
    def __call__(self, batch_text):
        ...
        '''
        batch_pos_text = self.word_repetition_normal(batch_text)  # self-repeat
        batch_neg_text = self.negative_samples(batch_text)  # get negative queue ; update queue
